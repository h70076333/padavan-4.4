...
#define BNDSTRG_DRIVER_VER	"3.1.1"
...
INT BndStrg_Init(PRTMP_ADAPTER pAd)
{
	...
	EnableRadioChstats(pAd, mac_val); //configure register for channel busy time
	return ret_val;
}
	...
	if (table->WhiteEntryListSize > 0) {
		for (i = 0; i < BND_STRG_MAX_WHITELIST_ENTRY; i++) {
			if (memcmp(table->WhiteEntryList[i], pSrcAddr, MAC_ADDR_LEN) == 0) {
				BND_STRG_MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("BndStrg STA %02x:%02x:%02x:%02x:%02x:%02x  whitelisted\n", PRINT_MAC(pSrcAddr)));
					return TRUE;
			}
		}
	}

	Rssi[0] = Elem->Rssi0 ? ConvertToRssi(pAd, Elem->Rssi0, RSSI_0) : 0;
	Rssi[1] = Elem->Rssi1 ? ConvertToRssi(pAd, Elem->Rssi1, RSSI_1) : 0;
	Rssi[2] = Elem->Rssi2 ? ConvertToRssi(pAd, Elem->Rssi2, RSSI_2) : 0;
	...
	if (table->BndStrgBlackListSize > 0) {
		for (i = 0; i < BND_STRG_MAX_BLACKLIST_ENTRY; i++) {
			if (memcmp(table->BndStrgBlackList[i], pSrcAddr, MAC_ADDR_LEN) == 0) {
				...
				("BndStrg STA %02x:%02x:%02x:%02x:%02x:%02x  blacklisted\n", PRINT_MAC(pSrcAddr)));
				return FALSE;
			}
		}
	}
...
VOID BndStrg_UpdateWhiteBlackList(PRTMP_ADAPTER pAd, PBND_STRG_CLI_TABLE table, BNDSTRG_MSG *msg)
{
		...
		BND_STRG_MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
						  ("%s: client[%02x:%02x:%02x:%02x:%02x:%02x] %s to %s\n",
						  __func__, PRINT_MAC(update_list->Addr), (update_list->deladd ? "Add":"Remove"), ((update_list->list_type == bndstrg_whitelist) ? "WhiteList" : "BlackList")));
	...
	PBND_STRG_CLI_TABLE table)
{
	...
	hex_dump("BndStrg_send_BTM_req Event length", (unsigned char *)Buf, Event->u.BTM_REQ_DATA.BTMReqLen);
